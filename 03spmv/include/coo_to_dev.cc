...
static int coo_to_dev(sparse_t& A) {
  ...
  A.coo.elems_dev=(coo_elem_t*)dev_malloc(sizeof(coo_elem_t)*A.nnz);
  to_dev((void*)A.coo.elems_dev, (void*)A.coo.elems, sizeof(coo_elem_t)*A.nnz);
  
  //A.coo.row_start_dev=(idx_t*)dev_malloc(sizeof(idx_t)*A.M);
  //to_dev((void*)A.csr.row_start_dev, (void*)A.csr.row_start, sizeof(idx_t)*A.M);
  return 1;
}
